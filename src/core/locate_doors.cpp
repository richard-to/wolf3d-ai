...
void wolf3d::hsv_threshold_doors(Mat frame, Mat &out)
{
    Vec3b hsv;    
    ...
    for (int i = kGameHBegin; i < h; ++i) {
        for (int j = kGameWBegin; j < w; ++j) {
            ...
            if (hsv[0] > kDoorHueMin && hsv[0] < kDoorHueMax) {
...
bool wolf3d::com_doors(Mat labelled, Mat &out, int label, wolf3d::COM_Meta *meta)
{
    int h = labelled.rows;
    int w = labelled.cols;
    ...
    if (black_percent > kDoorHistPercentMin && black_percent < kDoorHistPercentMax 
            && dim_ratio > kDoorDimDiffRatioMin) {
        rectangle(out,
            Point(left_j, top_i),
